...
defineFeature(feature, (test) => {
  ...
  test('Deleting a product if it is draft', ({ when, then, and, given }) => {
  ...
  test('Archiving a product it was already approved', ({
    ...
    then(
      ...
      async (productId: string) => {
        product = await getProductById(productReadRepository, productId);
      },
    );

    but(
      'the product should be marked as archived',
      async () => {
        expect(product.getPropsCopy().status).toEqual(ProductStatus.ARCHIVED);
      },
    );
  });
